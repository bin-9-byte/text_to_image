...
def create_text_image(
    ...
    width=None, height=None,  # 新增：图片分辨率（宽/高）
    ...
    total_text_height = sum(line_heights) + (len(line_heights) - 1) * line_spacing  # 总文字高度（含行间距）
...
def main():
    parser = argparse.ArgumentParser(description='文字转图片（支持分辨率、对齐方式、颜色控制）')
    # 基础参数
    parser.add_argument('--text', type=str, required=True, help='要渲染的文字（换行用\\n）')
    parser.add_argument('--font', type=str, required=True, help='字体文件路径（.ttf/.otf）')
    parser.add_argument('--output', type=str, default='output.png', help='输出图片路径（默认output.png）')
    # 新增：分辨率控制
    parser.add_argument('--width', type=int, help='图片宽度（像素），不指定则自动计算')
    parser.add_argument('--height', type=int, help='图片高度（像素），不指定则自动计算')
    # 新增：对齐方式
    parser.add_argument('--horizontal-align', type=str, default='center', 
                        choices=['left', 'center', 'right'], help='文字水平对齐（默认center）')
    parser.add_argument('--vertical-align', type=str, default='center', 
                        choices=['top', 'center', 'bottom'], help='文字垂直对齐（默认center）')
    # 原有参数（优化说明）
    parser.add_argument('--font-size', type=int, default=40, help='字体大小（默认40）')
    parser.add_argument('--text-color', type=str, default='0,0,0', 
                        help='文字颜色（RGB格式，如"255,0,0"为红色，默认黑色）')
    parser.add_argument('--bg-color', type=str, default='255,255,255', 
                        help='背景颜色（RGB格式，默认白色）')
    parser.add_argument('--padding', type=int, default=20, help='文字与图片边缘的间距（默认20）')
    ...
    try:
        ...
        assert all(0 <= c <= 255 for c in text_color) and all(0 <= c <= 255 for c in bg_color)
    except:
